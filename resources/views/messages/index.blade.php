...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-400 mt-1">Your conversations</p>
    </div>
    ...
    <div class="bg-[#1e293b] rounded-xl shadow-sm border border-[#374151] overflow-hidden">
        @forelse($conversations as $conversation)
            <a href="{{ route('messages.show', $conversation->partner->id) }}"
               class="block p-4 hover:bg-[#374151]/50 transition {{ !$loop->last ? 'border-b border-[#374151]' : '' }}">
                <div class="flex items-center space-x-4">
                    <div class="relative flex-shrink-0">
                        <img src="{{ $conversation->partner->avatar }}" alt="" class="w-12 h-12 rounded-full">
                        @if($conversation->unread_count > 0)
                            <span class="absolute -top-1 -right-1 w-5 h-5 bg-[#d4a853] text-[#0f1419] text-xs rounded-full flex items-center justify-center font-medium">
