...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
            ...
            <p class="mt-1 text-gray-500">Manage your product listings</p>
        </div>
        <a href="{{ route('farmer.products.create') }}" class="mt-4 sm:mt-0 inline-flex items-center px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 transition-colors shadow-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow group">
            ...
            <div class="aspect-w-4 aspect-h-3 bg-gray-100 relative overflow-hidden">
                ...
                <div class="absolute top-3 right-3">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $product->status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600' }}">
            ...
            <div class="p-4">
                ...
                <p class="text-xs text-gray-500 mt-1">{{ $product->category->name ?? 'Uncategorized' }}</p>
                ...
                <div class="mt-3 flex items-center justify-between">
                    ...
                    <span class="text-xs text-gray-500">Stock: {{ $product->quantity_available }}</span>
                </div>

                {{-- Actions --}}
                <div class="mt-4 flex items-center space-x-2">
                    <a href="{{ route('farmer.products.edit', $product->id) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 text-sm font-medium text-emerald-700 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition-colors">
                    ...
                    <form action="{{ route('farmer.products.destroy', $product->id) }}" method="POST" class="inline" x-data @submit.prevent="if(confirm('Are you sure you want to delete this product?')) $el.submit()">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="inline-flex items-center justify-center px-3 py-2 text-sm font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100 transition-colors">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-12 text-center">
        ...
        <a href="{{ route('farmer.products.create') }}" class="inline-flex items-center px-5 py-2.5 bg-emerald-600 text-white font-medium rounded-lg hover:bg-emerald-700 transition-colors">
            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"/>
            </svg>
